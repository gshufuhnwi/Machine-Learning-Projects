...
class RBF:
    ...
    def train(self):
        ...
        while epoch < 100:
            ...
            # monitor training set loss
            #predicted = self.test(self.inputLayer)
            #loss = self.calcLoss(self.inputClass, predicted)
...
# testing
#dataset = 'machine'        
#train = pd.read_csv('reduced_datasets/trainset_' + dataset + '.csv')
#prot = pd.read_csv('reduced_datasets/pam_' + dataset + '.csv')
#test = pd.read_csv('reduced_datasets/testset_' + dataset + '.csv')
#classification = False
#classes = 1

# normalize data
#for i in range(len(train.columns)-1):
#train.iloc[:,0:len(train.columns)-1] = preprocessing.scale(train.iloc[:,0:len(train.columns)-1].values)
#prot.iloc[:,0:len(train.columns)-1] = preprocessing.scale(prot.iloc[:,0:len(train.columns)-1].values)

#for col in prot.columns:
#    prot[col] = preprocessing.scale(prot[col].values)
#if (classification):
#    pass
#else:
#    train.iloc[:,len(train.columns)-1] = preprocessing.scale(train.iloc[:,len(train.columns)-1].values)

#rbf = RBF(train, prot, classification, classes)
#rbf.train()

#testClass = test[test.columns[-1]]
#testInputLayer = test.drop([test.columns[-1]], axis = 'columns')
#if(classification):
#    pass
#else:
#    testClass = preprocessing.scale(testClass)
#for col in testInputLayer.columns:
#    testInputLayer[col] = preprocessing.scale(testInputLayer[col].values)

#predicted = rbf.test(testInputLayer)
#loss = rbf.calcLoss(testClass, predicted)

...
and convert it into datafiles. PreprocessingData() handles
the categorial values and class reorder. NormalizeData() normalizes
all of the dataset as we are using sigomoid function. There is another
...
class LoadDataset:
        ...
        def get1sthiddenlayernode(self, key):
                #define SAE hidden layer 1 node number based on dataset
                #and tuned to get best performance.
                dict_list = {'abalone': 8, 'car': 5, 'segmentation': 12, 'machine': 5,
                        'forestfires': 10, 'winequality-red': 8, 'winequality-white': 8}
        ...
        def get2ndhiddenlayernode(self, key):
                ...
                #best performance.
                dict_list = {'abalone': [8,8], 'car': [5,5], 'segmentation': [10, 10], 'machine': [4,4] ,
                        'forestfires': [8,8], 'winequality-red': [6,6], 'winequality-white': [8, 8]}
                return dict_list.get(key)
        
        def get3rdhiddenlayernode(self, key):
                #define node number for 2 layers for each dataset and tune to get 
                #best performance.
                dict_list = {'abalone': [8, 6, 8], 'car': [5, 3, 5], 'segmentation': [10, 5, 10], 'machine': [5,3,5] ,
                        'forestfires': [8, 5, 8], 'winequality-red': [6, 3, 6], 'winequality-white': [8, 4, 8]}
                return dict_list.get(key)
